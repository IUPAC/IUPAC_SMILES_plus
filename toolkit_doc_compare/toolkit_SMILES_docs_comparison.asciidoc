...
https://chemapps.stolaf.edu/jmol/docs/misc/Jmol-OpenSmiles.pdf[Jmol SMILES Aromaticity Algorithm]

Jmol supports four aromaticity models including an internal Jmol 3D model (5-7 atom flat ring, with sp^2^ hybridization),
OpenSMILES (Hueckel's rule allowing exocylic double bonds), strict (no exocyclic double bonds), and MMFF94 model (excludes contributing
heteroatoms).
