...
v0.1,2020-09-29: Work in progress draft - incomplete and not approved yet                                   +
...
`%nnn` or `%(nnn)`, start with `nnn` as `100`.
...
Finally, if there are conflicting ring closure bonds defined such as 
`C=1CCCCC#1`, `C-1CCCCC=1`, or `c=1ccccc:1`, the SMILES are invalid. 
...
The dot bond cannot be used in front of a ring-closure digit. For example, `C.1CCCCC.1` is invalid.
...
carriage-return), or by the end of the string. As a result, and leading space in a SMILES string
is considered invalid in IUPAC SMILES+ (e.g. ` CCC`). Further an empty SMILES string,`''`, is invalid.
...
| 1.0 | 2020-09-28 | Clarified atom charge property syntax and added support for repeated symbols up to 15 and a 0 charge | Vincent F. Scalfani
| 1.0 | 2020-09-28 | Isotope changes: A 0 isotope is now undefined, leading 0 (e.g., 02) is invalid, and removed D,T symbols  | Vincent F. Scalfani
| 1.0 | 2020-09-28 | Clarified Wildcard atom section with `#0` and more examples.  | Vincent F. Scalfani
| 1.0 | 2020-09-28 | Removed lowercase as sp2 outside of rings  | Vincent F. Scalfani
| 1.0 | 2020-09-29 | Reworked and simplified SMILES flavors section into the Canonical SMILES section | Vincent F. Scalfani
| 1.0 | 2020-09-29 | Atom class change: no leading 0 allowed | Vincent F. Scalfani
| 1.0 | 2020-09-29 | Incorporated Nonstandard forms of SMILES section into main text. These are now considered invalid. | Vincent F. Scalfani
| 1.0 | 2020-09-29 | Ring rnum change: no leading 0 allowed and clarified when a conflict arises | Vincent F. Scalfani
| 1.0 | 2020-09-29 | Clarified that escaped up/down configurations are invalid | Vincent F. Scalfani
| 1.0 | 2020-09-29 | Added that leading spaces in SMILES are invalid as well as empty strings | Vincent F. Scalfani
| 1.0 | 2020-09-29 | Specified a strict bracket atom order | Vincent F. Scalfani
|======================
