= IUPAC SMILES+ Specification [work in progress draft, specification is incomplete and not approved yet]
:toc: macro
...
`[Mg2+]` is invalid. Moreover, providing multiple charge specifications such as `[Li+1-2] or [Li+-] is invalid. 
