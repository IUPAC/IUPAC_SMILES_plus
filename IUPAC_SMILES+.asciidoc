...
2+| **BRACKET ATOMS**
.6+| <<inatoms,Bracket Atoms>>      | _bracket_atom_ ::= `'['` _isotope_? _symbol_ _chiral_? _hcount_? _charge_? _class_? `']'`
...
an atom can also be represented by its atomic number preceded by `#` and enclosed in square brackets, `[]`.
...
The symbol `*` or `#0` are accepted as a valid atomic symbols, and represents a "wildcard" or unknown atom.
